...
describe("Store", () => {
  ...
  describe("state isolation", () => {
    it("should clone initialState to prevent mutations leaking back into the store fallback", () => {
      ...
      // Storage is still empty — must still read from the unmodified initialState clone.
    ...
    it("should clone stored data to prevent mutations leaking back into the store", () => {
      ...
      // Mutation of the returned snapshot must not affect the stored value.
  ...
  describe("validate option", () => {
    ...
    it("should fall back to initialState and warn when the validator rejects stored value", () => {
      ...
      // Write an incompatible shape directly, bypassing the typed store API.
    ...
    it("should accept stored values without a validator (no validate option)", () => {
