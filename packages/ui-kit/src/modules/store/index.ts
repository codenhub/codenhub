...
export function createStore<TSchema extends object>(
  ...
  // Accessing `localStorage` (not just `typeof localStorage`) can throw in some
  // restricted contexts (e.g. Safari private browsing, cross-origin iframes), so
  // the check itself is wrapped in a try/catch. All read/write paths go through
  // this guard to avoid duplicating it.
  ...
  // Returns true when the state was successfully persisted to localStorage,
  // false when storage is unavailable or the write throws (e.g. QuotaExceededError).
