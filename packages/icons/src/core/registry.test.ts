...
describe("IconRegistry", () => {
  ...
  it("does not resolve an unprefixed name without a default prefix", () => {
    const registry = new IconRegistry({ semanticAliases: false });
    registry.registerFamily(createFamily());
  ...
  it("resolves a semantic name to the family it points at", () => {
    const registry = new IconRegistry({ semanticAliases: { dismiss: "test:x" } });
    registry.registerFamily(createFamily());

    expect(registry.resolve("dismiss")?.name).toBe("test:x");
  });

  it("prefers a semantic name over the default prefix", () => {
    const registry = new IconRegistry({ defaultPrefix: "test", semanticAliases: { heart: "test:x" } });
    registry.registerFamily(createFamily());
  ...
  it("falls back to the default prefix when a semantic name carries no prefix", () => {
    ...
    expect(registry.resolve("dismiss")).toBeUndefined();
  });
