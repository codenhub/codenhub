  ...
  try {
    ...
    for (const { key } of probes.filter((probe) => probe.readsBg)) {
      ...
      /* `:hover` matches immediately, but the `background-color: var(--_bg)`
         ...
         `.box-hover`-won value until the browser's next style flush --
         `getComputedStyle()` read right after `.hover()` resolves still
         reports the resting value. A tick is enough; polling would be more
         precise but this is a one-shot generator, not a latency-sensitive one. */
      await page.waitForTimeout(30);
      const hovered = await page.evaluate((probeKey) => {
  ...
  try {
    ...
    try {
      const [light, dark] = await Promise.all(
        THEMES.map((theme) => readTheme(browser, stylesheetPath, theme, temporaryRoot)),
      );
