 ...
 * without writing it, exiting non-zero when it is stale -- what the CI drift
 * gate checks. Progress goes to stderr; the outcome is reported on stdout.
 */
...
import { applyGenerated } from "@codenhub/tools/generators";
...
/** Indents a block one level, for declarations nested inside `@layer theme`. */
...
async function main() {
  ...
  const [outcome] = await applyGenerated([{ contents: output, path: "src/palette.css" }], {
    dryRun: isDryRun,
    root: packageRoot,
  });

  if (isDryRun) {
    console.log(outcome.hasDrift ? "src/palette.css is out of date." : "src/palette.css is up to date.");
    if (outcome.hasDrift) {
      process.exitCode = 1;
    }
    return;
  ...
  console.log(outcome.hasDrift ? "src/palette.css written." : "src/palette.css is already up to date.");
}
