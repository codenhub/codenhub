...
- The Tailwind source entrypoints' `dark:` variant tracks the same rule: it fires under an explicit dark selector, or under the system preference when the subtree carries no explicit selector at all. An explicit selector at any depth still wins over the system preference, in either direction -- see [Setup → Dark variant](./setup.md#dark-variant).
