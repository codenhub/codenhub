...
Per-element shape classes (`.cut-diagonal`, `.cut-hex`) were considered as the way to expose the presets and deferred to [Structure for 0.5.0](./structure.md), where they are now the corner pattern: a knob covers everything under the aesthetic, and a single element can already take a shape through `--ui-radius` set inline.
...
- Scanlines or any painted background. They need a painted layer the model does not expose; [Structure for 0.5.0](./structure.md) added it as `--ui-surface-image`, which cyber can take as an additive change.
