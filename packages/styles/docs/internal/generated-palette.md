...
- **Only `.soft` and `.ghost`'s `bg`/`edge` actually vary by ground.** These are the only cells that need the three-ground split below.

The closed, already-established ground set from `registry.json`'s own component defaults is what gets baked for those cells -- not an open set, and not a new one invented for this proposal: `transparent` (`.badge`/`.btn`'s ground, the default, no suffix), `--color-background` (`.alert`'s ground via `--ui-surface-ground`, suffix `page`), and `--intent-subtle` (`.pre`/`.code`/`.kbd`/`.tooltip-bubble`'s ground, suffix `subtle`). A `.ghost` cell's `bg` on the `transparent` ground is `transparent` itself, and on `page`/`subtle` is exactly that ground token -- trivial values, but generated for naming consistency with `.soft`'s non-trivial ones rather than special-cased away.
