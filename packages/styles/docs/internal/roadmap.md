...
- **Boundary contrast.** Which boundaries the package owes WCAG 1.4.11 -- text controls, toggles, and `.progress` -- and the composition that keeps them at 3:1 for every intent, theme, and aesthetic. Measuring found more than the three recorded defects: every text field's resting line is under 3:1 for most intents, `.glass` and `.chunky-tile` replace control boundaries with a surface line, and `.progress`'s value misses 3:1 against its own track. Proposed in [Boundary contrast](./boundary-contrast.md), with one open question on what a field's hover moves. In review.
