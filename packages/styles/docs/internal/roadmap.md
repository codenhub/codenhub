...
- **Spurious utilities from the self-scan**: the compiled builds have no markup to scan, so Tailwind's content detection sees only this package's own files -- which is how the composition utilities (`box`, `surface`, `text-control`, `input-group`, `loader-mask`) emit as standalone rules, since nothing `@source inline`s them. The cost is that a few Tailwind utilities whose names collide with a CSS property or a comment word (`.backdrop-filter`, `.filter`, `.transition`, `.transform`, `.container`, ...) ship unused. `source(none)` plus an explicit `@source inline` of every composition utility would remove them, but the enumeration is easy to get wrong and a miss silently drops a real rule. Left as-is: the extras are inert and small. Recorded so the question is not reopened blind.
