...
- **Control boundaries under `.glass` and `.chunky-tile`.** Both replace every control's neutral line with their surface ink, leaving a neutral field at 1.02:1 (glass, light) and 1.65:1 (chunky tile) against the page, and a glass checkbox at 1.45:1. A control ink separate from the surface ink would let each keep its surface line; whether they take one, and at what value, is being reviewed on screen. See [Boundary contrast](./boundary-contrast.md#open-question-glass-and-chunky-tile-controls). In review.
