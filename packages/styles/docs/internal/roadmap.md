...
- **Boundary contrast.** The ring a neutral component with a line draws over its own translucent plate (1.54:1 in light, 1.81:1 in dark on `.btn.solid.edged`, `.badge`, and `.quote`): the line should fade out as the fill fills in rather than blend toward a plate that already runs under it. Proposed in [Boundary contrast](./boundary-contrast.md), which also records what was measured and not taken -- a field's line resting whole, which leaves hover and focus nothing to move, and `.progress` resting as an outline, where the value already carries the reading -- and one question on `.glass` and `.chunky-tile` controls. In review.
- **A stylesheet that survives inlining.** The loader art's SVG data URIs carry a literal `</style>`, from the `<style>` blocks that animate them, in every entry that ships the loader (`.`, `/components`, `/native`, and the `/tw` loader). A stylesheet linked or imported is unaffected, but one pasted into an HTML `<style>` element -- critical-CSS inlining, a framework's inline styles, a server-rendered head -- ends at that sequence, and every rule after the first loader art is dropped, the form seams included, with no error. Escape it in each data URI (`%3C/style%3E`), and fail the build when any built entrypoint contains `</style`. Not started.
