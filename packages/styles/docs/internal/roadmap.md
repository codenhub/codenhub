...
The second screen -- a dense, real form (`form/`) and a settings/detail screen (`settings/`) -- is built and read under every aesthetic and both themes. Two findings have landed: `.input-group`'s icon-tuned gap (now a `--input-group-gap` token) and a real model change to the three toggles, where a checked control used to vary by presentation into three different "on" identities while every unchecked one read the same washed-out plate close to the disabled look -- `:checked` now pins the fill and ink to one plate, `checkbox`/`radio` raise their resting cap to `40%` to separate `.ghost`/`.soft`/`.solid`, `.ghost` is no longer unsupported on any toggle, and the unclassed default moved from `.solid` (the loudest of the three separated plates) to `.soft`. Two findings are open pending a decision, not yet fixed: `.alert`/`.panel`'s soft-edged border misses 3:1 for some intents, which ones depending on the theme; and whether `.panel` (and possibly `.pre`/`.code`/`.kbd`, which ground on an already-tinted base) should join `.card`/`data-table`'s untinted-at-rest treatment, or take a different fix -- `.alert`, `.btn`, and `.progress` read fine as they are, so the question is scoped to containers used as repeated page sections. See [findings](./stress-test-findings.md) for both.
