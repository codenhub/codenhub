...
- **Spurious utilities from the self-scan**: the compiled builds have no markup to scan, so Tailwind's content detection sees only `src/` -- which is how the composition utilities (`box`, `surface`, `text-control`, `input-group`, `loader-mask`) emit as standalone rules, since nothing `@source inline`s them. The `@source not` lines in the four compiled entrypoints now exclude everything outside `src/` (the sibling apps, `docs/`, `scripts/`, and the root prose and data files), so a class name or `--color-*` reference that appears only in a documentation example no longer ships -- `tests/integration/exports.test.ts` guards the palette case. What remains is that a few Tailwind utilities whose names collide with a CSS property or a comment word (`.backdrop-filter`, `.filter`, `.transition`, `.transform`, `.container`, ...) still ship unused from the `src/` scan itself. `source(none)` plus an explicit `@source inline` of every composition utility would remove those too, but the enumeration is easy to get wrong and a miss silently drops a real rule. Left as-is: the extras are inert and small. Recorded so the question is not reopened blind.
