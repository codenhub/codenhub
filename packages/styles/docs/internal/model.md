...
Being unitless is what makes it safe to inherit, so a container can flatten a whole toolbar with one class and any element inside can opt back in.
...
**R8. A state re-declares an input, never a painted property.** It sets `--ui-fill`, `--ui-border`, or an intent slot, and lets `box` paint. It does not write `background-color`, `border-color`, or `box-shadow` of its own.
...
- Ink follows the theme, not the palette: near-black on light, near-white on dark. Pure black disappears on a dark page.
