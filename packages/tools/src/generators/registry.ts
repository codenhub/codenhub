 ...
 *
 * A generator specific to one package, such as `@codenhub/icons`' family data
 * or `@codenhub/styles`' palette, is that package's own `generate` script
 * instead -- `hub generate` runs it the same way it runs any other package
 * script, alongside these.
 * @returns Generators in execution order.
 */
export function createGenerators(): Generator[] {
  return [createLlmsFullGenerator(), createReadmePackagesGenerator(), createReferenceGenerator()];
}
