...
export interface DefinePageRouteOptions {
  /** Route path using the same syntax as the core router. */
  path: string;
  ...
  /** Optional document title or title factory for matched pages. */
...
export interface PageRouter {
  /** Navigates to an app-local path and returns the matched page route when one exists; throws during active navigation. */
...
export type PageContext<TRoute extends PageRoute = PageRoute> = PageMatch<TRoute> & {
  ...
  page: HTMLElement;
  /** Restricted navigation helper exposed to page code. */
  router: PageRouter;
};
...
export interface MountRouterLinkOptions {
  /** Root that receives delegated click handling. */
  root?: ParentNode;
...
export interface MountedRouter {
  /** Starts browser history integration and renders the current page when one matches; throws during active navigation. */
  start(): PageMatch | null;
  /** Navigates to an app-local path and renders a matching page; throws when the target is invalid or navigation is already running. */
  navigate(to: string, options?: NavigateOptions): PageMatch | null;
  /** Matches an app-local path without side effects; throws when the target is invalid. */
  match(to: string): PageMatch | null;
  /** Builds a browser href for an app-local path without navigating; throws when the target is invalid. */
  ...
  /** Removes router-owned listeners, disconnects links, destroys the active page, and clears the outlet. */
  destroy(): void;
}
