...
`clear(handle?)` dismisses all visible toasts when called with no arguments, or dismisses the specified toast when given a handle. `dismiss(handle?)` is an intuitive alias that calls `handle.dismiss()` when a handle is supplied or `clear()` when called without arguments.
...
- `toaster.clear(handle?)`: Dismisses all visible toasts, or a specific toast if a handle is passed.
- `toaster.dismiss(handle?)`: Alias for `handle.dismiss()` when a handle is passed or `clear()` when omitted.
