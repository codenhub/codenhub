...
A push to `main` says nothing about whether a package's new documentation has actually reached npm — two packages can be mid-flight together, one released and one not, and the watch-path filter above cannot tell them apart. So `src/lib/catalog.ts` does not read `packages/*/docs` from the working tree in production. Before a production build, `apps/docs/src/lib/published-docs-snapshot-integration.ts` writes a parallel tree, `apps/docs/.codenhub-published-docs/`, and `catalog.ts` reads from it instead: each package's `docs/` comes from `git show <tag>:...` at that package's own newest `<name>@<version>` tag, resolved by `@codenhub/tools/release`. A package with no tag yet — or whose docs did not exist yet at its latest tag — falls back to its live `docs/`, since there is nothing published to gate it against. The package manifest travels live and unpinned; only documentation content is scoped to a release. Eligibility — which packages get a snapshot at all — is the same `codenhub.docs` opt-in `buildPackageDefinitions` already checks, not whether a `docs/` directory currently exists, so a package whose live `docs/` was removed still resolves its latest tag rather than being skipped outright.

A git failure partway through resolving a tag's content is never treated the same as "nothing published there" — that would silently fall back to live, unreleased content for a package that has, in fact, released, which is the one outcome this mechanism exists to prevent. `buildPublishedDocsSnapshot` fails the build instead: an unreadable tag, a listed file that cannot be read, or a failed tag fetch all throw rather than resolve to an empty result.

This does not yet cover every public-facing surface. `src/lib/documentation-integration.ts` publishes a separate set of resources — `LICENSE`, `NOTICE`, and non-Markdown `docs/` assets such as diagrams — and validates each package's actual npm pack contents while doing it; it still reads the live `packagesRoot`, unpinned to any tag. Scoping it the same way `catalog.ts` is scoped would mean materializing a full historical package build at each tag, not just copying files, which is the same complexity that keeps `apps/demo` out of this change. Documentation prose — the part most likely to describe genuinely unreleased behavior — is covered; a diagram or a license file lagging behind is the accepted gap.

This only runs for `astro build` (`import.meta.env.PROD`), never `astro dev`, so editing a package's docs locally still hot-reloads against the working tree exactly as before.
