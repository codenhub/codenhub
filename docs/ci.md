...
### Publish-scoped content

A push to `main` says nothing about whether a package's new documentation has actually reached npm — two packages can be mid-flight together, one released and one not, and the watch-path filter above cannot tell them apart. So a production build does not read `packages/` from the working tree at all. Before it, `apps/docs/src/lib/published-docs-snapshot-integration.ts` writes a parallel tree, `apps/docs/.codenhub-published-docs/packages/`, and both readers of package content use it instead: `src/lib/catalog.ts` for pages, manifests, and the package list, and `src/lib/documentation-integration.ts` for `LICENSE`, `NOTICE`, and non-Markdown `docs/` assets, including the npm-pack validation it runs on them.

Each package in that tree is its own directory at its own newest `<name>@<version>` tag, resolved by `@codenhub/tools/release` and written out by `materializeTreeAtRef`, which stages the tagged tree into a throwaway git index and checks it out from there — byte-for-byte, binary files included, without touching the repository's index or working tree. It is then pruned to what the site publishes or validates: `package.json`, `README.md`, `LICENSE`, `NOTICE`, `llms.txt`, `llms-full.txt`, and public `docs/`. Source, tests, and configs are removed because the tree sits inside this app, where Vitest, `tsc`, and Vite would otherwise pick them up. Public docs cannot link outside that set (`invalid-docs-escape`), so nothing the site renders loses a link target; a README or `llms.txt` that linked into `src/` would fail the build with `missing-target`, and none does today.

A package reaches the site only when both hold: it has a release tag, and its manifest **at that tag** declares `codenhub.docs`. There is no fallback to the working tree. A package with no tag is absent, not shown live — falling back is how unreleased documentation used to reach the site, since most packages published before the tag workflow existed and had no tag to gate them. The opt-in is read from the released manifest because everything else is: a package joins the site in the release that ships its docs.

A failure is never treated as "nothing published there". A failed tag fetch, an unreadable tag, a package no longer at the location its tag put it, or a tag holding a different package at that location all fail the build rather than drop the package, because a released package missing from the site is a defect to report, not a state to ship. A package that moves directories therefore needs a release from its new location before the site can build again.

This only happens in a production build, never `astro dev`, so editing a package's docs locally still hot-reloads against the working tree. The switch is `readsPublishedSnapshot`, which mirrors `import.meta.env.PROD` — the signal `catalog.ts` chooses its glob roots on — rather than the Astro command alone: an `astro build` started under Vitest, as `src/layouts/docs-layout.test.ts` does, runs with `NODE_ENV=test`, reads the working tree for its pages, and must read it for resources too.

Because content is now scoped to what each package has actually published, the event that should refresh the site is a publish, not a merge — `.github/workflows/publish.yml` calls the `codenhub-docs` project's Cloudflare deploy hook once `hub publish` succeeds, so a release reaches the site without waiting for the next incidental push to `main` to trip the watch-path filter above. The push-triggered build still runs on top of that and re-derives the same tag-scoped snapshot every time, so it can never show more than what has actually published; it stays the only trigger for site content that lives outside `packages/*/docs` entirely, such as `apps/www`'s catalog and `apps/docs`'s own hand-authored shell pages.

One limitation this accepts: a documentation-only fix to an already-published package — a typo, a clarification, nothing behavior-affecting — is scoped to that package's `docs/` the same as everything else, so it does not reach the site until the package's next release either. Neither trigger rescues it, because both read the same tag-scoped snapshot. Revisit this if that turns out to matter in practice; it has not been separated out here to keep this a docs-only, single-purpose change.
...
None for publishing itself. The `publish` job holds `id-token: write` and authenticates through npm trusted publishing, which exchanges that OIDC token for a credential valid for the length of the publish. No npm token exists in this repository or in its Actions secrets, which is the same split the deployments above keep — the repository carries the build, never the key. The workflow's default permission is `contents: read`; `contents: write` is the `release` job's alone.

One secret does exist, and it is not for publishing: `CLOUDFLARE_DOCS_DEPLOY_HOOK_URL` is the `codenhub-docs` project's Cloudflare deploy hook, called once `hub publish` succeeds (see "Publish-scoped content" above). It can only trigger a rebuild of that one project — it cannot read or change anything — so it does not carry the weight the rest of this section is protecting against, but it is a new line in this ledger and is recorded here rather than left to be discovered in the workflow file. A missing or unreachable hook only warns; it never fails the job, since the version is already on npm by the time this step runs.
