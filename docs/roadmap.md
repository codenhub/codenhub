...
- [ ] Give an inlined icon an intrinsic size. `renderSvg` emits only a `viewBox`, so an SVG in svg mode has no size of its own, and the `1em` box it used to get came from the base stylesheet's `i[class^="ic-"]` and `.ic` rules, which stop matching once the `<i>` is gone. Emitting `width="1em" height="1em"` on the element fixes it for every consumer with no action on their part and no stylesheet, and attributes lose to CSS so anyone already sizing icons is unaffected. Worth doing on its own, ahead of the size axis below — `apps/demo` carries sizing rules that exist only to stand in for it
- [ ] Add a size axis, `ic-xs` through `ic-xl`, resolving to rem values. The default stays `--ic-size` with its `1em` fallback, so an icon with no size class behaves exactly as it does now and nothing breaks for a consumer relying on icons scaling with surrounding text; a size class sets `--ic-size` instead of competing with it. In svg mode the token resolves at build time into the `width` and `height` attributes above, which keeps that mode free of a stylesheet rather than making it depend on one — see Notes for the grammar decision this needs first
...
- The size axis above needs one decision made before it is written, and it is a grammar decision rather than a styling one: `ic-sm` today means the icon named `sm` through the default prefix, so a size token carves a hole in the icon-name space. Three readings were weighed. A sibling class (`ic-sm`) reads best and collides only with a family that genuinely ships an icon called `sm`; the modifier form (`ic-heart/sm`) matches the precedent `ic-heart/1.5` set and cannot collide, but it does not compose with a stroke modifier as the parser is written and repeats the size on every class; `ic-size-sm` barely collides and reads worst. The sibling class is the recommendation, on the condition that the collision is made loud rather than silent: the registry knows every icon name it holds, so the build can warn when a registered family actually contains one called `sm`. Whichever wins, the token is read by the scanner, the generator, the `vite`, `postcss`, and `tailwind` adapters, and the static `dist/style.css`, so the change is small in lines and wide in blast radius.
- Preserving `ic-` classes on an inlined SVG is safe and was checked rather than assumed: a per-icon rule sets only `--ic-uri` and `--ic-mask`, and the rule that reads them targets `i[class^="ic-"]` and `.ic`, neither of which an `<svg>` matches. A preserved class is inert today, which is what makes it available as a styling hook.
