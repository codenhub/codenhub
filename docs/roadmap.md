...
- [ ] Rework `apps/docs` navigation, primarily the labelling: top-level sections (hand-authored docs, then Reference, then Changelog), and a clearer scheme for what sidebar entries are named — the current flat two-level tree does not read well as generated content grows. Open, not designed — see Notes
