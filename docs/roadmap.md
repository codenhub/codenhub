...
- [ ] Support `mode: "svg"` in an Astro host. SVG rewriting runs in `transformIndexHtml` and in `transform`, and the latter is gated on `MARKUP_FILE`, which covers `.html`, `.js(x)`, `.ts(x)`, `.vue`, and `.svelte`. `.astro` is in neither, and `content` does not help — it widens which files are scanned for class names, not which are rewritten — so an Astro project cannot get inline SVG out of the plugin at all today, and `docs/frameworks/astro.md` sends it to CSS mode instead. Wanted for `apps/demo`, which is the first Astro host in this repository to use the package — see Notes
...
- `@codenhub/icons` has one blind spot with two symptoms, both found by deploying `apps/demo`: the plugin reaches an HTML page only through `transformIndexHtml`, which Astro never runs for its own pages. In CSS mode that meant the generated stylesheet was never injected, which an explicit `virtual:icons.css` import now solves and `docs/frameworks/astro.md` documents. In svg mode there is no equivalent escape hatch, which is the roadmap entry above. Whichever way svg support is added, it will not need that hook.
- An icon class that resolves in no registered family produces no rule in CSS mode and is reported by nothing, in any host — the unresolved-class warning belongs to svg mode alone. `ic-warning` sat broken in `apps/demo` with a green build for that reason, because Lucide, its default prefix, has no `warning`. Giving CSS mode an equivalent warning is worth doing and is not yet tracked as its own entry, because where it would be raised from is the open question: the scan sees classes one module at a time and has no point at which it knows the set is complete.
