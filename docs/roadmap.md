...
- [ ] Give an inlined icon a size. `renderSvg` emits only a `viewBox`, and the `1em` box an icon used to get comes from base rules matching `i[class^="ic-"]` and `.ic`, which stop matching once svg mode replaces the `<i>`, so an inlined icon renders at 0×0 unless surrounding CSS happens to size it. Two ways to fix it and they are not exclusive: emit `width="1em" height="1em"` on the element, which needs no stylesheet, or add a geometry-only rule for `svg[class*="ic-"]` reading `--ic-size`, which keeps one mechanism across both modes and is what makes the size axis below work in svg mode. `apps/demo` carries sizing rules that exist only to stand in for this
- [ ] Add a size axis, `ic-xs` through `ic-xl`, resolving to rem values. A size class only sets `--ic-size`, which every existing form already reads, so it is one declaration per size and needs no knowledge of the element carrying it; the default stays `--ic-size`'s `1em` fallback. Sizing an inlined SVG is the entry above. The open question is the grammar — see Notes
...
- The size axis needs one decision made before it is written, and it is about the class grammar rather than about styling: `ic-sm` today means the icon named `sm` through the default prefix, so a size token carves a hole in the icon-name space. A sibling class (`ic-sm`) reads best and collides only with a family that genuinely ships an icon by that name; the modifier form (`ic-heart/sm`) follows the precedent `ic-heart/1.5` set and cannot collide, but does not compose with a stroke modifier as `parseIconClass` is written, and repeats the size on every class. The collision is the only real obstacle, and it can be made loud rather than silent: the registry knows every icon name it holds, so the build can say so when a registered family contains one. Whichever wins, the token is read by the scanner, the generator, the `vite`, `postcss`, and `tailwind` adapters, and the static `dist/style.css`; those have to agree or the same class means different things by delivery method.
- An `ic-` class already survives onto an inlined SVG, and is inert there: the rewriter keeps any such class it did not consume as the icon, a per-icon rule sets only `--ic-uri` and `--ic-mask`, and the rule reading those targets `i[class^="ic-"]` and `.ic`, which an `<svg>` does not match. So a size class needs no new plumbing to reach the element — only something that reads `--ic-size` on it. A bare `ic` is the exception: the rewriter drops it, so it is not available as a hook.
- The `<i>` in those selectors is load-bearing and unrelated to sizing. It separates an element that _is_ the icon from one that _has_ one: `:not(i, input, select, textarea, .ic)[class^="ic-"]::before` gives `<button class="ic-save">Save</button>` a glyph beside its label, and dropping the qualifier would paint the mask over the button instead. Narrowing the package to `<i>` only is a defensible simplification, but it removes the `::before`, `::after`, and form-control forms, so it is its own decision and not a step toward the entries above.
