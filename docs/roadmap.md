...
- `@codenhub/icons` has one blind spot with two symptoms, both found by deploying `apps/demo`: the plugin reaches an HTML page only through `transformIndexHtml`, which Astro never runs for its own pages. In CSS mode that meant the generated stylesheet was never injected, which an explicit `virtual:icons.css` import now solves and `docs/frameworks/astro.md` documents. In svg mode there is no equivalent escape hatch, which is the roadmap entry above. The same hook carries the unresolved-class warning, so an icon that resolves in no registered family is silently blank in an Astro host — `ic-warning` sat broken in `apps/demo` with a green build, because Lucide, its default prefix, has no `warning`. Whichever way svg support is added, it should also give that warning a path that does not depend on the HTML hook.
