...
- Each symbol section contains, in order:
  ...
  3. For a class or interface: its public members, each with its own signature block and, when `prose` is `true`, its TSDoc. A member inherited from another type is not repeated in full here: it is listed by name with a link to the type that declares it, and its signature and prose stay on that type's own section. A member inherited from a type outside the package's documented surface is dropped.
