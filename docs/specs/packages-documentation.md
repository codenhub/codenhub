...
`llms-full.txt` is generated by `pnpm generate`, as defined by `docs/tooling.md`,
and MUST NOT be edited by hand. Generated content identifies each source document
with a `<!-- Source: ... -->` marker and MUST NOT become a competing contract.
The generator rebases relative link destinations so they resolve from
`llms-full.txt`; this is the only allowed change to copied Markdown bodies.
Regenerate it in the same change as its inputs.
...
`pnpm check` enforces this section, as defined by `docs/tooling.md`. Tarball
publication is checked only under `pnpm check --pack`, because it requires real
npm output.
