...
A generator specific to one package, rather than the same logic applied uniformly across every package, is not registered in `packages/tools/src/generators/`: it is that package's own `generate` script. `@codenhub/styles`' `./palette` export and `@codenhub/icons`' committed family data both work this way — `packages/styles/scripts/generate-palette.mjs` and `packages/icons/scripts/generate-icon-data.ts` each call `applyGenerated` on themselves, the same function `hub generate`'s own in-process generators go through. `hub generate` dispatches to a selected package's `generate` script the way `hub test` or `hub typecheck` dispatch to theirs, forwarding `--dry-run` as an argument rather than skipping the run the way a dry `build` does, since the script still has to run to know whether it would write anything.
